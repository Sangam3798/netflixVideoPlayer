...
class CustomMediaControls extends StatefulWidget {
  const CustomMediaControls({
    required this.backgroundColor,
    required this.iconColor,
    required this.onTabSubtitleBtn,
    this.showPlayButton = true,
    this.videoTitle = '',
    this.supportedSubTitle = const [],
    Key? key,
  }) : super(key: key);
  ...
  final List<String> supportedSubTitle;
  final Function(int?) onTabSubtitleBtn;
...
class _CupertinoControlsState extends State<CustomMediaControls> with SingleTickerProviderStateMixin {
        ...
        child: ClipRRect(
          ...
          child: BackdropFilter(
            ...
            child: Container(
              ...
              child: Center(
                child: Icon(
                  chewieController.isFullScreen ? Icons.arrow_back : CupertinoIcons.arrow_up_left_arrow_down_right,
                  color: iconColor,
                  size: 16,
                ),
              ),
  ...
  Widget _buildSubtitleToggle(Color iconColor, double barHeight) {
    ...
    return GestureDetector(
      ...
      onTap: () async {
        if (widget.supportedSubTitle.isNotEmpty) {
          showCupertinoModalPopup(
              ...
              builder: (ctx) => CupertinoActionSheet(
                    actions: List.generate(
                        widget.supportedSubTitle.length,
                              ...
                              child: Row(
                                ...
                                children: [
                                  if (index == defaultSubtitleIndex)
                                    Icon(Icons.check, size: 20.0, color: CupertinoTheme.of(context).primaryColor),
                                  Text(widget.supportedSubTitle[index]),
                                ],
                              ),
                            )),
                  ));
      ...
      child: Row(
        children: <Widget>[
          ...
          AnimatedOpacity(
            opacity: notifier.hideStuff ? 0.0 : 1.0,
            duration: const Duration(milliseconds: 300),
            child: Text(widget.videoTitle, style: const TextStyle(color: Colors.white, backgroundColor: Colors.black12)),
          ),
          const Spacer(),
